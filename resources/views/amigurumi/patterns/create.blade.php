...
@section('content')
    ...
    <form method="POST" action="{{ route('amigurumi-patterns.store') }}">
        ...
        <div class="mb-3">
            ...
            <textarea name="yarn_description" id="yarn_description" class="form-control" rows="3" required>{{ old('yarn_description') }}</textarea>
        </div>

        <div class="mb-3">
            <label for="tools_description" class="form-label">{{ __('Tools Description') }}</label>
            <textarea name="tools_description" id="tools_description" class="form-control" rows="2" required>{{ old('tools_description') }}</textarea>
        </div>
